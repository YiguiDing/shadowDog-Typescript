...
import { Particle, Dust, FireImg, FireImgSpak, Explosion } from "./Particle.js";
...
export class ShadowDog extends Animal {
	...
	diveAudio: HTMLAudioElement;
	jumpWithFireAudio: HTMLAudioElement;
	jumpAudio: HTMLAudioElement;
	barkingAudio: HTMLAudioElement;
	dizzyAudio: HTMLAudioElement;
	DyingAudio: HTMLAudioElement;
	getHitAudio: HTMLAudioElement;
	constructor(Game: Game, CANVAS_WIDTH: number, CANVAS_HEIGHT: number) {
		super(
			"./imgs/shadow_dog.png",
			575,
			523,
	...
	isGetHit(enemys: Array<Animal>) {
		// return false;
		for (let index = 0; index < enemys.length; index++) {
			const enemy = enemys[index];
			if (enemy.getAliveFlag() && enemy.isCollision(this)) return true;
		}
		return false;
	}
...
class JumpingState extends State {
	...
	inputsHandler(inputs: Array<ValueOf<typeof InputListener.KeyMaps>>): void {
		if (this.shadowDog.isOnGround()) this.shadowDog.setState(ShadowDog.StateNamesEnum.Running);
		else if (this.shadowDog.isfalling()) this.shadowDog.changeAnimateByName("fall");
		else if (inputs.includes(InputListener.KeyMaps.PressSpase)) this.shadowDog.setState(ShadowDog.StateNamesEnum.Diving);
	...
	update() {
		...
		this.shadowDog.Game.Scene.particles.unshift(
			new FireImg(
				...
				this.shadowDog.posY + this.shadowDog.drawHeight / 2,
				-this.shadowDog.maxMoveSpeedX
			)
		);
	...
	inputsHandler(inputs: Array<ValueOf<typeof InputListener.KeyMaps>>): void {
		if (this.shadowDog.isOnGround()) {
			...
			for (let index = 0; index < this.shadowDog.Game.Scene.particlesMaxLength / 2; index++) {
				this.shadowDog.Game.Scene.spakParticles.unshift(
					new FireImgSpak(this.shadowDog.posX + this.shadowDog.drawWidth / 2, this.shadowDog.posY + this.shadowDog.drawHeight)
				);
			}
...
class SittingState extends State {
	...
	inputsHandler(inputs: Array<ValueOf<typeof InputListener.KeyMaps>>): void {
		if (inputs.length == 0) this.shadowDog.setState(ShadowDog.StateNamesEnum.Running);
		if (inputs.includes(InputListener.KeyMaps.PressRight)) this.shadowDog.setState(ShadowDog.StateNamesEnum.Barkting);
	}
}
...
class PreDizzyState extends State {
	...
	enter(): void {
		// 如果在地上就直接进入Dizzy状态
		if (this.shadowDog.isOnGround()) this.shadowDog.setState(ShadowDog.StateNamesEnum.Dizzy);
		// 否则呈现掉落动画
...
class DyingState extends State {
	...
	update(timeInterval: number): void {
		// 是最后一帧，播放完毕就结束游戏
		if (this.shadowDog.isLastAnimateFrame()) this.shadowDog.Game.setState(Game.StateEnum.GameOver);
	}
	inputsHandler(inputs: Array<ValueOf<typeof InputListener.KeyMaps>>): void {
		return;
	}
