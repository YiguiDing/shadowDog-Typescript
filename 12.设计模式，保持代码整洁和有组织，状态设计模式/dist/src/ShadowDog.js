...
export class ShadowDog extends Animal {
    constructor(Context2D, CANVAS_WIDTH, CANVAS_HEIGHT) {
        super(Context2D, CANVAS_WIDTH, CANVAS_HEIGHT, "./imgs/shadow_dog.png", 575, 523, 0.4, [7, 7, 7, 9, 11, 5, 7, 7, 12, 4], ["idle", "jump", "fall", "run", "dizzy", "sit", "roll", "bite", "ko", "gethit"]);
...
// 发呆状态
ShadowDog.InnerClass_IdleState = class InnerClass_IdleState extends State {
    ...
    inputsHandler(inputs) {
        if (inputs.includes(InputListener.KeyMaps.PressRight))
            this.shadowDog.setState(ShadowDog.StateNamesEnum.RunningRight);
        if (inputs.includes(InputListener.KeyMaps.PressUp))
            this.shadowDog.setState(ShadowDog.StateNamesEnum.JumpUp);
        if (inputs.includes(InputListener.KeyMaps.PressLeft))
            this.shadowDog.setState(ShadowDog.StateNamesEnum.MoveBack);
        if (inputs.includes(InputListener.KeyMaps.PressDown))
            this.shadowDog.setState(ShadowDog.StateNamesEnum.Sit);
    }
};
ShadowDog.InnerClass_RunningRightState = class InnerClass_RunningRightState extends State {
    constructor(shadowDog) {
    ...
    inputsHandler(inputs) {
        if (inputs.length == 0)
            this.shadowDog.setState(ShadowDog.StateNamesEnum.Idle);
    }
};
ShadowDog.InnerClass_MoveBackState = class InnerClass_MoveBackState extends State {
    constructor(shadowDog) {
        super(ShadowDog.StateNamesEnum.RunningRight);
        this.shadowDog = shadowDog;
    }
...
ShadowDog.InnerClass_JumpUpState = class InnerClass_JumpUpState extends State {
    constructor(shadowDog) {
        super(ShadowDog.StateNamesEnum.RunningRight);
        this.shadowDog = shadowDog;
    }
    ...
    inputsHandler(inputs) {
        if (this.shadowDog.isfalling())
            this.shadowDog.setState(ShadowDog.StateNamesEnum.JumpFall);
    }
};
ShadowDog.InnerClass_JumpFallState = class InnerClass_JumpFallState extends State {
    constructor(shadowDog) {
        super(ShadowDog.StateNamesEnum.RunningRight);
        this.shadowDog = shadowDog;
    }
...
ShadowDog.InnerClass_SetState = class InnerClass_SetState extends State {
    constructor(shadowDog) {
        super(ShadowDog.StateNamesEnum.RunningRight);
        this.shadowDog = shadowDog;
    }
